<script lang="ts">
  ...
  onMount(() => {
    ...
    const addHoverClasses = (event: MouseEvent) => {
      ...
      if (target.closest(".option, .social-card, .settings-button, .nav-button, .hamburger-button, .menu-item, .twaos-cardd")) {
        if (target.closest(".circle-no-interact")) return;
    ...
    const removeHoverClasses = (event: MouseEvent) => {
      ...
      if (target.closest(".option, .social-card, .settings-button, .nav-button, .hamburger-button, .menu-item, .twaos-card")) {
